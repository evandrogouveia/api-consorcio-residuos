...
module.exports = {
    ...
    getSearchCategories(req, res) {
        const term = req.query.term[0];

        const selectCategories = `SELECT * FROM psCategory WHERE 
        LOWER(psCategory.name) LIKE LOWER('%${term}%') OR
        LOWER(psCategory.exercise) LIKE LOWER('%${term}%')
        `;
